...
public class AuthValidator implements Validator {
    ...
    @Autowired
    public AuthValidator(BoardDAO boardDAO) { this.boardDAO = boardDAO; }
    ...
    public void validate(Object target, Errors errors){
        ...
        /** 닉네임 여부 확인 */
        if(boardVO.getUsername() == null || boardVO.getUsername().trim().isEmpty())
...
//        if(count != 0) errors.rejectValue("userid", "useridAlreadyExists");
        ...
        /* 비밀번호 검사 */
        if(boardVO.hasPassword()){
            /**
             *  서순 주의
             *  1) 비밀번호의 길이 검사
             *  2) 비밀번호 일치여부 확인
             *  */
            System.out.println("입력된 비밀번호: " + boardVO.getPassword() + ", 확인: " + boardVO.getPasswordConfirm());
            if(boardVO.getPassword().length() < 5)
                errors.rejectValue("password", "shortPassword");
            else if(!boardVO.isSamePasswordConfirmPassword())
...
//        if(!isRealnameKor) errors.rejectValue("realname", "notKorean");
         ...
         *  2) 길이 검사
         *  */
        final String username = boardVO.getUsername();
